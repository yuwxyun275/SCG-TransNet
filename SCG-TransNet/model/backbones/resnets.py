...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 6, 'input_size': (3, 256, 256), 'pool_size': (7, 7),
...
class ResNet(nn.Module):
    ...
    def __init__(
            ...
            drop_rate=0.0, drop_path_rate=0., drop_block_rate=0., zero_init_last=True, block_args=None, high_level=0):
        super(ResNet, self).__init__()
        print("Building a Custom ResNet!")
        print(high_level)
    ...
    def forward_features(self, x):
        ...
        x = self.layer1(x)
        low_level = self.low_level_proj(x).permute(0, 2, 3, 1).contiguous()
        # print(x.shape)
        x = self.layer2(x)
        # print(x.shape)
        x = self.layer3(x)
        high_level = self.high_level_proj(x).permute(0, 2, 3, 1).contiguous()
        # print(x.shape)
        # x = self.layer4(x)
        # print(x.shape)
...
def resnet50_encoder(pretrained=False, **kwargs):
    ...
    return _create_resnet('resnet50_encoder', pretrained, default_cfg=default_cfgs['resnet50_encoder'], **model_args)